  ...
  try {
    ...
    const data = await con
      ...
      .insertMany([
        {
          date: 2022,
          comment: 'Puikus buvo prisijungimas',
          user_id: '645a9645e3864ef67c31b1cc',
        },
      ]);
    await con.close();
    res.send(data);
...
app.post('/users', async (req, res) => {
  try {
    ...
    const con = await client.connect();
    const data = await con.db(dbName).collection('users').insertOne(comment)
    res.send(data);
  } catch (error) {
    res.status(500).send(error);
  }
...
app.post('/comments/:id', async (req, res) => {
  try {
    ...
    const data = await con
      ...
      .insertMany([
        {
          comment: 'Komentaras 8888885555555555555555',
          date: new Date(),
